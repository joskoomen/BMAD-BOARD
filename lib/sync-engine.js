...
class SyncEngine {
  ...
  _loadState() {
    try {
      ...
      console.warn(`[SyncEngine] Failed to load state from ${this.statePath}:`, err);
    }
    ...
    for (const remoteStory of (remoteData.stories || [])) {
      ...
      if (localChanged && remoteChanged) {
        ...
        if (conflictStrategy === 'remote-wins') {
          ...
          if (localStory.filePath && fs.existsSync(localStory.filePath)) {
            try { localMtime = fs.statSync(localStory.filePath).mtime; } catch { /* deleted */ }
          }
  ...
  _updateStoryStatus(slug, newStatus) {
    void this.scanProject; // referenced in constructor, not used here
