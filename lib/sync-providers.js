...
const notionProvider = {
  ...
  async push(config, syncState, projectData) {
    ...
    for (const epic of (projectData.epics || [])) {
      if (!isFirst) await delay(350); // Rate limit: ~3 req/sec
      isFirst = false;
    ...
    for (const epic of (projectData.epics || [])) {
      for (const story of (epic.stories || [])) {
        ...
        let pageId;
        if (existing?.remoteId) {
          // Atomic replace: create new page, archive old one
          ...
          if (blocks.length > 100) {
            await delay(350);
            await notion.blocks.children.append({ block_id: newPage.id, children: blocks.slice(100) });
          }
          // Archive the old page
          await delay(350);
          await notion.pages.update({ page_id: existing.remoteId, archived: true });
          pageId = newPage.id;
        } else {
          const page = await notion.pages.create({
            parent: { database_id: config.storiesDatabaseId },
    ...
    if (config.documentsPageId) {
      ...
      for (const child of children.results) {
        ...
        items.documents.push({
          name: title,
          filename: title.toLowerCase().replace(/\s+/g, '-') + `-${child.id}` + '.md',
          content,
          remoteId: child.id,
          lastEdited: page.last_edited_time
        });
...
const obsidianProvider = {
  ...
  async pull(config, syncState) {
    ...
    // Scope pull to the configured project folder (set during setup)
    // rather than scanning every top-level vault directory
    if (config.basePath) {
