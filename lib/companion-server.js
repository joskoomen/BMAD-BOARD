...
const STATIC_DIR = path.resolve(__dirname, '..', 'companion');
...
class CompanionServer {
   ...
   * @param {function} [opts.launchOnDesktop] - (command, storySlug, phase) => void — open terminal tab on desktop
   */
  constructor(opts) {
    ...
    this.launchOnDesktop = opts.launchOnDesktop || null;
  ...
  /**
   * Rotate the auth token and disconnect all existing clients.
   */
  rotateToken() {
    this.token = crypto.randomBytes(24).toString('hex');
    // Disconnect all clients so they must re-auth with new token
    for (const client of this.clients) {
  ...
  /**
   * Route incoming HTTP requests to API or static file handlers.
  ...
  /**
   * Dispatch authenticated REST API requests to the appropriate handler.
   * @param {http.IncomingMessage} req
   * @param {http.ServerResponse} res
  ...
  _handleAPI(req, res, url) {
    ...
    if (pathname === '/api/epics' && req.method === 'GET') {
      ...
      const data = this.scanProject(projectPath);
      const lite = this._lightProjectData(data);
      return this._json(res, { epics: lite.epics || [] });
    }
  ...
  /**
   * Serve static PWA files from the companion/ directory.
   * Falls back to index.html for SPA client-side routing.
   * @param {http.IncomingMessage} req
   * @param {http.ServerResponse} res
   * @param {string} pathname - URL pathname to serve
   */
  ...
  /**
   * Handle a new WebSocket connection: authenticate, send initial state,
   * wire message/close handlers, and set up heartbeat tracking.
   * @param {WebSocket} ws
   * @param {http.IncomingMessage} req
   */
  _handleWS(ws, req) {
    ...
    ws._authToken = this.token; // Store token used at connect time
    ...
    // Send current active stories so companion knows which tasks are running
   ...
   * Supports terminal CRUD, project refresh, shared terminal watching,
   * story advancement, and heartbeat pings.
   * @param {WebSocket} ws - The sending client
   * @param {{ type: string, data?: object }} msg - Parsed JSON message
   */
  _handleWSMessage(ws, msg) {
    switch (msg.type) {
      ...
      case 'terminal:input': {
        const sessions = this.clientTerminals.get(ws);
        if (msg.data?.id && msg.data?.input && sessions?.has(msg.data.id)) {
      ...
      case 'terminal:resize': {
        const sessions = this.clientTerminals.get(ws);
        if (msg.data?.id && msg.data?.cols && msg.data?.rows && sessions?.has(msg.data.id)) {
      ...
      case 'terminal:kill': {
        const sessions = this.clientTerminals.get(ws);
        if (msg.data?.id && sessions?.has(msg.data.id)) {
          this.terminalManager.kill(msg.data.id);
          sessions.delete(msg.data.id);
        }
        break;
      }
        ...
        if (currentIdx >= 0 && currentIdx < PHASE_ORDER.length - 1) {
          ...
          if (this.updateStoryStatus) {
            try {
              ...
              // Launch the phase command on the desktop terminal
              if (this.launchOnDesktop) {
                const cmd = this.buildCommand(newPhase, slug, targetStory.filePath);
                if (cmd) {
                  this.launchOnDesktop(cmd, slug, newPhase);
                  // Send task-launched feedback to originating client
                  ws.send(JSON.stringify({
                    type: 'story:task-launched',
                    data: { slug, phase: newPhase, command: cmd }
                  }));
                }
              }
            } catch (e) {
      ...
      case 'story:launch': {
        // Launch phase command on desktop WITHOUT advancing the phase
        const slug = msg.data?.slug;
        if (!slug) break;
        ...
        if (!targetStory) {
          ws.send(JSON.stringify({ type: 'error', data: { message: 'Story not found' } }));
          break;
        }

        if (this.launchOnDesktop && this.buildCommand) {
          const cmd = this.buildCommand(targetStory.status, slug, targetStory.filePath);
          if (cmd) {
            this.launchOnDesktop(cmd, slug, targetStory.status);
            ws.send(JSON.stringify({
              type: 'story:task-launched',
              data: { slug, phase: targetStory.status, command: cmd }
            }));
          } else {
            ws.send(JSON.stringify({
              type: 'error',
              data: { message: `No command available for phase: ${targetStory.status}` }
            }));
          }
        }
        break;
      ...
      case 'stories:list-active': {
        ws.send(JSON.stringify({
          type: 'stories:active',
          data: { stories: this.getActiveStories() }
        }));
        break;
      }
  ...
  /**
   * Handle POST /api/story/:slug/advance — move a story to its next phase.
   * Updates sprint-status.yaml and broadcasts the new state to all clients.
   * @param {http.IncomingMessage} req
   * @param {http.ServerResponse} res
   * @param {string} slug - Story slug identifier
   */
   ...
   */
  /**
   * Register a desktop terminal session with story metadata.
   * Called from main process when a story-associated terminal tab is created.
   */
  shareTerminalStart(sessionId, { storySlug, storyPhase, command } = {}) {
    this.sharedTerminals.set(sessionId, {
      buffer: '',
      active: true,
    ...
    this.broadcast('stories:active', { stories: this.getActiveStories() });
  }

  /**
   * Get list of stories that have active terminal sessions.
   */
  getActiveStories() {
    const result = [];
    for (const [id, session] of this.sharedTerminals) {
      if (session.active && session.storySlug) {
        result.push({
          slug: session.storySlug,
          phase: session.storyPhase,
          sessionId: id,
  ...
  shareTerminalData(sessionId, data) {
    ...
    if (!this.sharedTerminals.has(sessionId)) {
      this.sharedTerminals.set(sessionId, {
        buffer: '', active: true,
        storySlug: null, storyPhase: null, command: null, startedAt: Date.now()
      });
    }
    const session = this.sharedTerminals.get(sessionId);
    session.buffer += data;
  ...
  shareTerminalExit(sessionId, exitCode) {
    ...
    // Send notification about terminal exit (include story context if available)
    const storyInfo = session?.storySlug ? ` (${session.storySlug})` : '';
    this.broadcast('notification', {
      title: 'Terminal Exited',
      body: `Session${storyInfo} ended with code ${exitCode}`,
      type: exitCode === 0 ? 'success' : 'error'
    });

    // Broadcast updated active stories list
    this.broadcast('stories:active', { stories: this.getActiveStories() });
  }
    ...
    for (const [id, session] of this.sharedTerminals) {
      if (session.active) {
        result.push({
          ...
          storySlug: session.storySlug || null,
          storyPhase: session.storyPhase || null,
  ...
  /**
   * Verify request authentication via Bearer header or query token.
   * @param {http.IncomingMessage} req
   * @param {URL} url - Parsed request URL
  ...
  /**
   * Read the full request body with a 1MB size limit.
   * Destroys the socket if the limit is exceeded.
   * @param {http.IncomingMessage} req
   * @param {function(string): void} callback - Called with the body string
   */
  ...
  /**
   * Get all non-internal IPv4 addresses on the local machine.
...
/**
 * Start a 30-second heartbeat interval to detect and clean up dead WebSocket connections.
 * @param {CompanionServer} server
