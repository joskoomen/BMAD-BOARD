...
class TerminalManager {
  /**
   * @param {object} [opts]
   * @param {object} [opts.ptyProvider] - Override the PTY provider (for testing)
   */
  constructor(opts = {}) {
    this.sessions = new Map(); // id -> { pty, listeners }
    this.nextId = 1;
    this._pty = opts.ptyProvider || pty;
  }
  ...
  create({ cwd, cols = 120, rows = 30, onData, onExit }) {
    ...
    cols = Math.max(1, Math.floor(cols)) || 120;
    rows = Math.max(1, Math.floor(rows)) || 30;

    const ptyProcess = this._pty.spawn(shell, [], {
      name: 'xterm-256color',
      cols,
      rows,
      cwd: cwd || os.homedir(),
