...
/**
 * Parse command-line arguments for recognized options.
 *
 * Recognizes the --project-path <value> option and resolves it to an absolute path.
 * @param {string[]} argv - Command-line arguments array (process.argv-style).
 * @returns {{projectPath?: string}} An object with `projectPath` set to the resolved absolute path when provided; otherwise an empty object.
 */
...
/**
 * Start and run the MCP server for BMAD Board sync over stdio.
 *
 * Parses command-line arguments (supports --project-path), initializes the SyncEngine
 * for the target project, registers MCP tools and resources that expose sync operations
 * and project information, and connects the server using a stdio transport so it begins
 * serving requests.
 */
