...
/**
 * Get the configuration for a story phase.
 * @param {string} status - Phase key (e.g. 'backlog', 'in-progress')
 * @returns {{ label: string, color: string, icon: string, command: string|null, description: string }}
 */
...
/**
 * Get the zero-based index of a phase in the phase order.
 * @param {string} status - Phase key
 * @returns {number} Index (0 if unknown)
 */
...
/**
 * Get the next phase after the given status, or null if already done.
 * @param {string} status - Current phase key
 * @returns {string|null}
 */
...
/**
 * Build a Claude-specific command for a story phase (backward-compatible alias).
 * @param {string} phase
