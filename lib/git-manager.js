...
class GitManager {
  ...
  async branches() {
    ...
    // Get ahead/behind for each local branch with a tracking branch
    let trackingInfo = {};
    try {
      const { execSync } = require('child_process');
      const raw = execSync(
        'git for-each-ref --format="%(refname:short) %(upstream:short) %(upstream:track)" refs/heads/',
        { cwd: this.projectPath, stdio: ['pipe', 'pipe', 'pipe'] }
      ).toString().trim();
      for (const line of raw.split('\n')) {
        if (!line) continue;
        const parts = line.match(/^(\S+)\s+(\S*)\s*(.*)$/);
        if (!parts) continue;
        const [, branchName, upstream, track] = parts;
  ...
  /** Discard changes in a specific file (restore to HEAD). */
  ...
  /** Amend the last commit with currently staged changes and/or new message. */
  async amend(message) {
    const { execSync } = require('child_process');
    if (message) {
    ...
    // Get the amended commit hash
    const hash = execSync('git rev-parse HEAD', { cwd: this.projectPath }).toString().trim();
    return { hash };
  }
  ...
  async rebase(branch) {
    try {
      ...
      return { success: false, conflicts: status.conflicted || [], message: err.message };
    }
  }
  ...
  async isRebasing() {
    const fs = require('fs');
    const path = require('path');
    return fs.existsSync(path.join(this.projectPath, '.git', 'rebase-merge')) ||
           fs.existsSync(path.join(this.projectPath, '.git', 'rebase-apply'));
  }

  // ── File History ──────────────────────────────────────────────────────

  /** Get commit history for a specific file. */
  async fileLog(file, limit = 25) {
    try {
      const result = await this.git.log({ maxCount: limit, file });
      return (result.all || []).map(c => ({
        hash: c.hash,
        hashShort: c.hash.substring(0, 7),
        date: c.date,
  ...
  /** Read a file from the working tree (for conflict viewer). */
  async readConflictFile(file) {
    const fs = require('fs');
  ...
  async resolveConflict(file, content) {
    const fs = require('fs');
    const filePath = require('path').join(this.projectPath, file);
    fs.writeFileSync(filePath, content, 'utf8');
    await this.stage([file]);
  }
}
